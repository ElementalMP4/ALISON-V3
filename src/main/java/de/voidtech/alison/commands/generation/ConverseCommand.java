...
public class ConverseCommand extends AbstractCommand {
    ...
    public void execute(CommandContext commandContext, List<String> args) {
        ...
        if (args.isEmpty()) {
            if (privacyService.userHasOptedOut(commandContext.getAuthor().getId())) {
            ...
            startSentence = textGenerationService.createConversationPrompt(commandContext.getAuthor().getId());
    ...
    private String getGavinResponse(String message) {
        try {
            ...
            URL requestURL = new URL(config.getGavinUrl());
